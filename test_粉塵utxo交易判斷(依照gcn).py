...
def preprocess_transactions(json_data):
    ...
    for item in json_data:
        ...
        for outdetail in output_details:
            ...
            output_amount = float(outdetail['amount'])
            if output_amount <= 0.00001:
...
def process_chunk(chunk_data):
    ...
    normal_transactions = [
        ...
        if item['Dust Bool'] == '0' and 100 >= float(item['Txn Output Address']) >= 3
        and any(
            input_detail.get('cluster') == '-1' 
            for input_detail in json.loads(item['Txn Input Details'])
        )
    ]
...
def main():
    file_name = 'BTX_Transaction_data_2024_01_18_12-13.json'
    ...
    # 先過濾出正常交易的數量
    normal_txn_count = sum(1 for item in json_data if item['Dust Bool'] == '0')
    print(f"總交易數: {len(json_data)}")
    print(f"正常交易數: {normal_txn_count}")
